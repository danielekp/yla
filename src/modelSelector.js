...
/**
 * Handles the model download process including UI updates and progress tracking
 * @param {Event} event - The click event that triggered the download
 * @param {string} modelName - Name of the model to download
 */
          ...
          if (status === 'pulling manifest') {
            ...
            if (data.digest) {
              ...
              if (totalAllLayers > 0) {
                ...
                statusElement.textContent = `Downloading ${formatBytes(
                  completedAllLayers
                )} of ${formatBytes(totalAllLayers)} (${progress}%)`;
              }
            }
 ...
 * @param {string} modelName - Name of the model whose download should be cancelled
 */
 ...
 * @returns {string} - Formatted string with appropriate unit (Bytes, KB, MB, GB, TB)
 */
...
/**
 * Handles model selection and updates UI accordingly
 * @param {Object} model - The selected model configuration
 */
    ...
    mainContent.innerHTML = `
            ...
            <div class="system-message">
                ...
                <span class="system-text">${
                  model.systemMessage || 'No system message defined'
                }</span>
            </div>
        `;
