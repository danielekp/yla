...
const ModelSelector = (function () {
      ...
      if (status === 'pulling manifest') {
        ...
        if (progress.totalAllLayers > 0) {
          const progressPercent = Math.round(
            (progress.completedAllLayers / progress.totalAllLayers) * 100
          );
          statusMessage = `Downloading ${this.formatBytes(
            progress.completedAllLayers
          )} of ${this.formatBytes(progress.totalAllLayers)} (${progressPercent}%)`;
        }
  ...
  const UIManager = {
    ...
    resetDownloadUI(elements, isError = false, errorMessage = '') {
      const { downloadButton, progressBar, progressContainer, statusElement, cancelButton } =
        elements;
      ...
      while (true) {
        ...
        for (const chunk of chunks) {
          ...
          try {
            ...
            const progressUpdate = DownloadManager.processDownloadProgress(data, layers, progress);

            // Update UI with progress
            const progressPercent =
              progress.totalAllLayers > 0
            ...
            if (progressUpdate.isComplete) {
              UIManager.updateDownloadProgress(uiElements, 100, 'Download complete!');
